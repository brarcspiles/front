...
export default function Reports() {
    ...
    const fetchTotalReceivedAmount = async () => {
        try {
            ...
            const response = await fetch(`https://server-chi-seven-94.vercel.app/api/currentMonthReceivedAmount2/${userid}?startOfMonth=${moment(startDate).format('YYYY-MM-DD')}&endOfMonth=${moment(endDate).format('YYYY-MM-DD')}`);
    ...
    const prepareChartData = (data) => {
        const labels = data.map(entry => moment(entry._id).format('YYYY-MM')); // Format date for labels
