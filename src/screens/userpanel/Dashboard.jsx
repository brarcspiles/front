...
export default function Dashboard() {
  ...
  const fetchCurMonReceivedAmount = async () => {
    try {
      const userid = localStorage.getItem("userid");
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://server-three-mu-70.vercel.app/api/currentMonthReceivedAmount/${userid}`, {
        headers: {
          'Authorization': authToken,
        }
      });
  ...
  const fetchTotalPaymentsReceived = async () => {
    try {
        const authToken = localStorage.getItem('authToken');
        const userId = localStorage.getItem('userid');
        const response = await fetch(`https://server-three-mu-70.vercel.app/api/totalPaymentReceived/${userId}`, {
            headers: {
                Authorization: authToken,
            },
        });
  ...
  //     const authToken = localStorage.getItem('authToken');
  //     const userid = localStorage.getItem('userid');
  //     const response = await fetch(`https://server-three-mu-70.vercel.app/api/totalPaymentsReceived/${userid}`, {
  //       headers: {
  //         Authorization: authToken,
  //       },
  //     });
  ...
  //     }
  //     const json = await response.json();
  //     setTotalPaymentsReceived(json.totalPaymentsReceived); // Update state here
  //     setloading(false); // Consider if you need this here
  //   } catch (error) {
  //     console.error('Error fetching data:', error);
  //   }
