...
export default function Estimatedetail() {
  ...
  const fetchestimateData = async () => {
    try {
      ...
      const response = await fetch(`https://server-chi-seven-94.vercel.app/api/getestimatedata/${estimateid}`, {
        headers: {
  ...
  const checkCustomerSignature = async (estimateIdpass) => {
    ...
    try {
      const response = await fetch(`https://server-chi-seven-94.vercel.app/api/checkcustomersignature/${encodeURIComponent(estimateIdpass)}`);
      const json = await response.json();
  ...
  const fetchOwnerData = async () => {
    try {
      ...
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://server-chi-seven-94.vercel.app/api/getownerdata/${ownerId}`, {
        headers: {
          'Authorization': authToken,
        }
      });
  ...
  const fetchtransactiondata = async () => {
    try {
      const userid = localStorage.getItem("userid");
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://server-chi-seven-94.vercel.app/api/gettransactiondata/${estimateid}`, {
        headers: {
          'Authorization': authToken,
        }
      });
...
.text-end {
  ...
  const handleRemove = async (estimateid, estimateIdpass) => {
    try {
      ...
      if (signatureData) {
        const authToken = localStorage.getItem('authToken');
        const deleteSignatureResponse = await fetch(`https://server-chi-seven-94.vercel.app/api/delcustomersignature/${encodeURIComponent(estimateIdpass)}`, {
          method: 'DELETE',
          headers: {
            'Authorization': authToken,
          }
      ...
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://server-chi-seven-94.vercel.app/api/delestimatedata/${estimateid}`, {
        method: 'GET',
        headers: {
          'Authorization': authToken,
        }
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch('https://server-chi-seven-94.vercel.app/api/send-estimate-email', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        await fetch(`https://server-chi-seven-94.vercel.app/api/updateestimateData/${estimateid}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': authToken,
      ...
      const checkResponse = await fetch(`https://server-chi-seven-94.vercel.app/api/checkcustomersignature/${encodeURIComponent(estimateData._id)}`);
      const checkJson = await checkResponse.json();

      if (checkResponse.ok && !checkJson.hasSignature) {
        // Create new customer signature only if it doesn't exist
        await fetch('https://server-chi-seven-94.vercel.app/api/customersignature', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            // 'Authorization': authToken,
