...
const WaiverRequestList = () => {
  ...
  useEffect(() => {
    ...
    const fetchWaivers = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(`https://server-three-mu-70.vercel.app/api/getAllWaivers?userId=${encodeURIComponent(userId)}`, {
          method: 'GET',
          headers: {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this waiver?")) {
      try {
        const response = await fetch(`https://server-three-mu-70.vercel.app/api/deleteWaiver?id=${id}`, {
          method: 'DELETE',
        });
