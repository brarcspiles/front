    ...
    const handleClockIn = async () => {
        try {
          ...
          const authToken = localStorage.getItem('authToken');
            ...
            const response = await fetch('https://server-5pxf.onrender.com/api/clockin', {
              method: 'POST',
              headers: {
        ...
        const handleClockOut = async () => {
            try {
              ...
              let isTeamMember = localStorage.getItem('isTeamMember');
              const authToken = localStorage.getItem('authToken');
              const response = await fetch('https://server-5pxf.onrender.com/api/clockout', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': authToken,
          ...
          const fetchUserEntries = async (start, end) => {
            try {
              const userid = localStorage.getItem('userid');
              const authToken = localStorage.getItem('authToken');
              const response = await fetch(`https://server-5pxf.onrender.com/api/userEntries/${userid}`, {
                headers: {
                  'Authorization': authToken,
                }
              });
